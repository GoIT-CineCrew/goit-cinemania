...
const paginationContainer = document.querySelector(".catalog-pages .catalog-pages-list"); 
...
function createMovieCard(movie) {
    ...
    const year = movie.release_date ? movie.release_date.slice(0, 4) : "Rating not Found";
...
function renderPaginationButtons(total_pages, current_page) {
    ...
    paginationContainer.insertAdjacentHTML("beforeend", `
        <li>
            <button type="button" class="catalog-page-btn prev-btn" ${currentPage === 1 ? 'disabled' : ''}>
                <svg>
                    <use href="./img/sprite.svg#back-arrow-${currentPage === 1 ? 'passive' : 'active'}-dark-mobile"></use>
                </svg>
            </button>
        </li>
    `);
    ...
    if (startPage > 1) {
        paginationContainer.insertAdjacentHTML("beforeend", createPageButton(1, currentPage));
        if (startPage > 2) {
             paginationContainer.insertAdjacentHTML("beforeend", `
                <li>
    ...
    for (let i = startPage; i <= endPage; i++) {
        paginationContainer.insertAdjacentHTML("beforeend", createPageButton(i, currentPage));
    }
    ...
    if (endPage < totalPages) {
        ...
        paginationContainer.insertAdjacentHTML("beforeend", createPageButton(totalPages, currentPage));
    }

    // Sonraki Butonu
    paginationContainer.insertAdjacentHTML("beforeend", `
        <li>
            <button type="button" class="catalog-page-btn next-btn" ${currentPage === totalPages ? 'disabled' : ''}>
                <svg>
                    <use href="./img/sprite.svg#forward-arrow-${currentPage === totalPages ? 'passive' : 'active'}-dark-mobile"></use>
                </svg>
            </button>
        </li>
    `);
...
function createPageButton(pageNumber, currentPage) {
    ...
    const useHref = isActive ? "button-active-dark-mobile" : "button-passive-dark-mobile";
    return `
        <li>
            <button type="button" class="catalog-page-btn page-number-btn ${isActive ? 'active' : ''}" data-page="${pageNumber}">
                <svg>
                    <use href="./img/sprite.svg#${useHref}"></use>
                    <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-size="10" fill="${isActive ? 'white' : 'currentColor'}">${pageNumber.toString().padStart(2, '0')}</text>
                </svg>
            </button>
        </li>
    `;
...
clearBtn.addEventListener("click", () => {
    ...
    const defaultOption = Array.from(optionsList).find(o => o.dataset.value === undefined || o.textContent.trim() === "Year");
    if (defaultOption) {
